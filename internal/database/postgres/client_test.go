...
func testPostgresConfig(t *testing.T) Config {
	t.Helper()

	port := 5432
	if rawPort := lookupEnv("POSTGRES_TEST_PORT", "PGSQL_TEST_PORT"); rawPort != "" {
		parsed, err := strconv.Atoi(rawPort)
		if err != nil {
			t.Fatalf("invalid POSTGRES_TEST_PORT: %v", err)
		}
		port = parsed
	}

	config := Config{
		Host:     lookupEnv("POSTGRES_TEST_HOST", "PGSQL_TEST_HOST"),
		Port:     port,
		User:     lookupEnv("POSTGRES_TEST_USER", "PGSQL_TEST_USER"),
		Password: lookupEnv("POSTGRES_TEST_PASSWORD", "PGSQL_TEST_PASSWORD"),
		Database: lookupEnv("POSTGRES_TEST_DATABASE", "PGSQL_TEST_DATABASE"),
		SSLMode:  lookupEnv("POSTGRES_TEST_SSLMODE", "PGSQL_TEST_SSLMODE"),
	}
	if config.SSLMode == "" {
		config.SSLMode = "disable"
	}
	if config.Host == "" || config.User == "" || config.Database == "" {
		t.Skip("skipping PostgreSQL integration tests; set POSTGRES_TEST_HOST, POSTGRES_TEST_USER, POSTGRES_TEST_PASSWORD, POSTGRES_TEST_DATABASE")
	}
...
func TestNewClient(t *testing.T) {
	client, err := NewClient(testPostgresConfig(t))
	if err != nil {
...
func TestClientDDLDMLOperations(t *testing.T) {
	client, err := NewClient(testPostgresConfig(t))
	if err != nil {
		t.Skipf("跳过测试 - 无法连接到PostgreSQL: %v", err)
		return
...
func TestClientTransactionSupport(t *testing.T) {
	client, err := NewClient(testPostgresConfig(t))
	if err != nil {
		t.Skipf("跳过测试 - 无法连接到PostgreSQL: %v", err)
		return

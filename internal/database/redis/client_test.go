...
func testConfig(t *testing.T) Config {
	...
	if rawDB := os.Getenv("REDIS_TEST_DB"); rawDB != "" {
	...
	if config.Addr == "" {
		t.Skip("skipping Redis integration tests; set REDIS_TEST_ADDR and optional REDIS_TEST_PASSWORD/REDIS_TEST_DB")
	}
...
func TestParseCommand(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			expected: []interface{}{"SET", "greeting", "hello world"},
		},
		{
			input:    `SET note 'it\'s fine'`,
			expected: []interface{}{"SET", "note", "it's fine"},
		},
	}
...
func TestParseCommandRejectsUnterminatedQuote(t *testing.T) {
	if _, err := ParseCommand(`SET greeting "hello world`); err == nil {
